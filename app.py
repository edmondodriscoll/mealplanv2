...
APP_TITLE = 'Macro-Aware Meal Planner (Google Sheets)'
...
def load_data_from_gsheet(sheet_url: str, worksheet: str | None):
    ...
    ws = sh.worksheet(worksheet) if (worksheet and worksheet.strip()) else sh.sheet1
...
def ensure_state():
    ...
    st.session_state.setdefault('sheet_url', st.secrets.get('gsheets', {}).get('url', ''))
    st.session_state.setdefault('worksheet', st.secrets.get('gsheets', {}).get('worksheet', ''))
...
def write_saved(plans):
    SAVED_FILE.write_text(json.dumps(plans, indent=2), encoding='utf-8')
...
def main():
    ...
    with tabs[0]:
        with st.sidebar:
            ...
            st.caption('Defaults can come from secrets; you can override here.')
            st.text_input('Sheet URL', key='sheet_url', placeholder='https://docs.google.com/spreadsheets/d/...')
            st.text_input('Worksheet (optional)', key='worksheet', placeholder='Sheet1')
        ...
        if not st.session_state['sheet_url']:
            st.error('No Google Sheet URL configured. Add it to secrets or enter it in the sidebar.')
            st.stop()

        try:
            df = load_data_from_gsheet(st.session_state['sheet_url'], st.session_state.get('worksheet',''))
